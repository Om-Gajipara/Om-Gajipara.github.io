...
function Faculty() {
    ...
    const [faculty , setFac]=useState(faculty1 )
    function deleteCard(index){
      faculty.splice(index,1)
        setFac([...faculty])
    }
    //display fac
    const [displayFaculty , setDisplayFac]=useState(faculty1 )
    function displayCard(index){
        faculty.splice(index,1)
        setDisplayFac(faculty[index])
        console.log(faculty[index])
    }

    let FacultyCard = faculty.map((faculty , index)=>{
      return (
          <div class="card col-2 p-1 m-3">
              ...
              <div class="card-body">
                ...
                <p class="card-text">{faculty.FacultyDesignation}</p>
                <p class="card-text">{faculty.FacultyEducationQualification}</p>
                <p class="card-text"><b>Exp:</b> {faculty.FacultyExperience}</p>
                <p class="card-text"><b>Working since:</b> {faculty.FacultyWorkingSince}</p>
                <b/>
                <button className="btn btn-primary" onClick={()=>{deleteCard(index)}}>delete</button>{' '}
                
                <button className="btn btn-primary" onClick={()=>{displayCard(index)}}>Display</button>
              </div>
          </div>
        )
    });
